...
(* Answer: The [apply] tactic applies a theorem or lemma to the current
    goal, while the [rewrite] tactic tries to match the current goal
    against a hypothesis or lemma in the context and then replaces
    the goal with the conclusion of that hypothesis or lemma.

    The main difference between the two tactics is that [apply] can
    apply a theorem or lemma that is not in the context, while [rewrite]
    can only apply a hypothesis or lemma that is already in the
    context.

    The situations where both can usefully be applied are:

    - When we want to apply a theorem or lemma that is not in the
      context, but we do not want to introduce any new hypotheses.
...
Proof.
  ...
  (* rewrite <- H12 in H2.
  injection H2 as H21.
  apply trans_eq with (m:=z).
  apply H11.  symmetry. apply H21.
Qed. *)
  apply trans_eq with (n:= x) (m:=z) (o:= y).
  apply H11.
  symmetry in H2.
  apply trans_eq with (o:= y::l) in H2.
  injection H2 as H21.
  apply H21.
  rewrite H12.
  reflexivity.
...
Proof.
  ...
  - (* n = S n' *) intros m eq. destruct m as [| m'] eqn:E.
    + (* m = O *) discriminate eq.
    + (* m = S m' *) f_equal. apply IHn'. simpl in eq. apply eq.
Qed.
...
Proof.
  ...
  - (* m = O *) simpl. intros eq. destruct n as [| n'] eqn:E.
    + (* n = O *) reflexivity.
    + (* n = S n' *) simpl in *. discriminate eq.
...
Proof.
  intros f b.
  destruct (f true) as [|] eqn:E1; destruct (f false) as [|] eqn:E2; destruct b.
...
Theorem eqb_true_sym : forall (n m : nat),
  n = m -> n =? m = true.
Proof.
  intros n.
  induction n as [| n IHn].
  ...
  - simpl. intros m. destruct m as [| m'].
    + discriminate.
    + intros H. apply IHn. injection H as H1. apply H1.
Qed.
...
Proof.
  ...
  apply trans_eq with (o:=p) in H1.
  - apply eqb_true_sym. apply H1.
  - apply H2.
Qed. 
...
Proof.
  ...
  intros X Y l. 
  induction l as [| (x,y) l' IHl'].
  - simpl. intros l1 l2 H H1 H2.
    apply length_0 in H1.
    apply length_0 in H2.
    rewrite <- H1. rewrite <- H2. reflexivity.
  - simpl. intros [|x' l1'] [|y' l2'] H H1 H2.
    ...
    injection H1 as H1.
    injection H2 as H2.
    injection H as E1 E2 H.
    assert(goal : split l' = (l1', l2')).
    { apply IHl'. apply H. apply H1. apply H2. }
    rewrite goal. rewrite E1. rewrite E2. reflexivity.
Qed.
...
(* Theorem double_filter : forall (X : Type) (test : X -> bool) (l : list X),
...
Proof.
  ...
  - simpl. destruct (test x) as [|] eqn:E.
    + simpl. rewrite E. f_equal. apply IHl'.
    + simpl. apply IHl'.
Qed.

Theorem double_filter_length : forall (X : Type) (test : X -> bool) (l : list X),
  length (filter test (filter test l)) = length (filter test l).
Proof.
  intros X test l.
...
Theorem filter_test : forall (X : Type) (test : X -> bool) (l : list X),
  length l <=? length (filter test l) = true ->
  filter test l = l.
Proof. Admitted. *)

Lemma filter_neg: forall (X : Type) (test : X -> bool) (l : list X),
  filter (fun b => negb (test b)) (filter test l) = [].
Proof.
  intros X test l.
  induction l as [| x l' IHl'].
  - simpl. reflexivity.
  - simpl. destruct (test x) as [|] eqn:E.
    + simpl. rewrite E. f_equal. simpl. apply IHl'.
    + simpl. apply IHl'.  
Qed.
...
Qed.
  ...
  - simpl. reflexivity.
  - apply f_equal with (f:= filter (fun b => negb (test b))) in H as H1.
    simpl in H1. rewrite E in H1. simpl in H1.
    rewrite filter_neg in H1.
    discriminate H1.
Qed. *)
...
Proof. 
  ...
  - simpl. destruct (test x) as [|] eqn:E.
    ...
    + simpl. unfold existsb'. simpl. rewrite E. simpl. apply IHl'.
Qed.
