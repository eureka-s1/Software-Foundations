...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?  (Add YES or NO to each line.)
      ...
      - [e bool (b c 0)]
      - [c]  *)
(* Check (d (b a 5)) : grumble bool. *)
(* Check (d mumble (b a 5)) : grumble bool. *)
...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
  intros X l. induction l as [|h l' IHl']. simpl.
  - reflexivity.
...
Theorem map_distribute : forall (X Y: Type) (f : X -> Y) (l1 l2: list X),
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite map_distribute. rewrite IHl'. reflexivity.
Qed.
(** [] *)
...
Definition exp (n m : cnat) : cnat := 
  ...
  (* m (forall X : Type, (X -> X) -> X -> X) (mult n) one. *)
