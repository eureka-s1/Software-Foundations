...
(** The [assert] tactic introduces two sub-goals.  The first is
    ...
    assertion with curly braces [{ ... }], dboth for readability and so
...
Proof.
  ...
  assert (H2: n + m = m + n).
  { rewrite add_comm. reflexivity. }
  rewrite H2. rewrite add_assoc. reflexivity.  Qed.
  (** [] *)
...
Proof.
  ...
  - (* p = S p' *)
    ...
    { rewrite add_comm. reflexivity. }
    assert (H3: n + (n * p' + m) = n + n * p' + m).
    { rewrite <- add_assoc. reflexivity. }
    rewrite H1. rewrite H2. rewrite H3. reflexivity.  Qed.
...
Proof.
  ...
  - (* b = B0 n *)
    simpl.
    replace (bin_to_nat b0 + 0) with (bin_to_nat b0).
  ...
  - (* b = B1 n *)
    simpl. rewrite IHb1. 
    replace (1 + bin_to_nat b1 + 0) with ((bin_to_nat b1 + 0) + 1).
    replace (bin_to_nat b1 + 0) with (bin_to_nat b1).
    + rewrite add_assoc. reflexivity.
    + rewrite add_0_r. reflexivity.
    + rewrite add_0_r. rewrite add_0_r. rewrite add_comm. reflexivity.  
Qed.
...
Lemma double_bin_nat_to_bin : forall n,
...
Proof.
  ...
  - (* n = 0 *)
    reflexivity.
  - (* n = S n' *)
    simpl. rewrite add_comm. simpl. rewrite IHn'. 
    rewrite double_incr_bin.  reflexivity.  
Qed.

Lemma double_bin_plus_1_nat_to_bin : forall n,
...
Proof.
  ...
  - (* n = S n' *)
    ...
    assert (H: n' + S n' = n' + n' + 1).
    { rewrite <- add_assoc.  replace (n' + 1) with (1 + n'). 
      + reflexivity.
      + rewrite add_comm. reflexivity. }
    rewrite H.
    rewrite IHn'. 
...
Proof.
  ...
  - (* b = B0 n *)
    ...
    rewrite double_nat_to_bin. rewrite IHb0. destruct b0 as [| b0'| b0''].
    + reflexivity.

Failed : B0 Z equals Z, which is not the same as Z.
...
(* Leading 0? Like B0 Z equals Z, which is not the same as Z. *)
...
Proof.
  ...
  - (* b = B0 n *)
    simpl. rewrite add_0_r. 
    rewrite double_bin_nat_to_bin. rewrite IHb0. destruct b0 as [| b0'| b0''].
    + reflexivity.
    + simpl. reflexivity.
    + simpl. reflexivity.
  - (* b = B1 n *)
    simpl. rewrite add_0_r.
    rewrite double_bin_plus_1_nat_to_bin. rewrite IHb1. destruct b1 as [| b1'| b1''].
    + reflexivity.
    + simpl. reflexivity.
    + simpl. reflexivity.
Qed.
