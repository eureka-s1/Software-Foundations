...
Proof.
  ...
  destruct n as [|n'].
  - left. reflexivity.
  - destruct m as [|m']. right. reflexivity.
    discriminate.
Qed. 
(** [] *)
...
Proof.
  ...
  unfold not in *.
  split.
  - (* left *) intros HP. apply H. left. apply HP.
  - (* right *) intros HQ. apply H. right. apply HQ.
Qed.
...
Proof.
  (* WORKED IN CLASS *)
  intros n H. (* note the implicit [destruct] here *)
  destruct H as [m Hm].
  exists (2 + m).
  apply Hm.  Qed.
...
Proof.
  ...
  - (* n = S n' *)
    ...
    + (* m = S m' *)
      simpl in *.
      destruct Hm as [x Hm]. injection Hm as Hm.
      assert (goal : exists y : nat, m' = n' + y).
      { exists x. apply Hm. }
      apply Hn' in goal.
      apply goal.
Qed.
...
Proof.
  ...
  - induction l as [|x l' IHl'].
    ...
    + simpl. intros [H | H].
      ...
      { apply IHl' in H. 
        destruct H as [z [Hz1 Hz2]]. exists z. 
        split. apply Hz1. 
        right. apply Hz2. }
  - induction l as [|x l' IHl'].
    + simpl. intros []. destruct H as [_ F]. destruct F.
    + simpl. intros H. destruct H as [z [Hz1 [Hz2 | Hz3]]].
      { rewrite <- Hz2 in Hz1. left. apply Hz1. }
      { assert (goal : (exists x0 : A, f x0 = y /\ In x0 l')). 
        { exists z. split. apply Hz1. apply Hz3. } 
        apply IHl' in goal.
        right. apply goal. }
Qed.
(** [] *)
...
Proof.
  intros A l1. induction l1 as [|x1 l1' IHl1'].
  - simpl. intros l2 a. 
    split.
  ...
  - simpl. intros l2 a. 
    ...
    + intros H. destruct H as [H | H].
      { left. left. apply H. }
      { apply IHl1' in H. apply or_assoc. right. apply H. }
    + intros [H | H]. 
      { destruct H as [H | H]. left. apply H.
...
Proof.
  ...
  - simpl. split.
    + intros H. split.
      ...
      { apply IHl'. intros x' Hx'. apply H. right. apply Hx'. }
    + intros H. destruct H as [H1  H2]. intros x' [Hx' | Hx'].
      { rewrite Hx' in H1. apply H1. }
      { rewrite <- IHl' in H2. apply H2 in Hx'. apply Hx'. }
Qed.
...
Proof.
  ...
  - destruct (even n') as [|] eqn: E.
    + rewrite even_S. rewrite E. simpl.
      ...
      exists k. f_equal. apply IHk'.
    + rewrite even_S. rewrite E. simpl.
      destruct IHn' as [k IHk'].
      exists (1 + k). simpl. rewrite IHk'. reflexivity.
Qed.
...
Proof. 
  ...
  - simpl. split.
    ...
    + intros H. destruct H as [H | H]. apply H. apply H.
  - simpl. split.
    + intros H. left. apply H.
    + intros H. destruct H as [H | H]. apply H. discriminate.
  - simpl. split.
    + intros H. right. apply H.
    + intros H. destruct H as [H | H]. discriminate. apply H.
  - simpl. split.
    + intros H. discriminate.
    + intros H. destruct H as [H | H]. discriminate. discriminate.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite <- eqb_eq.
  split.
  - intros H. rewrite H. intros H'. discriminate.
  - intros H. destruct (x =? y) as [|] eqn : E.
...
Proof.
  ...
  induction l as [|x l' IHl'].
  - simpl. reflexivity.
  - simpl. apply andb_true_iff. split. apply H. reflexivity. apply IHl'.
Qed.
...
Proof.
  ...
  - generalize dependent l2.
    ...
    + simpl. destruct l2 as [| a2 l2'].
      { intros H'. discriminate. }
      { simpl. intros E. apply andb_true_iff in E.  
        destruct E as [Ea El].
        apply H in Ea.
        apply IHl1' in El.
        f_equal. apply Ea. apply El. }
  - intros E. rewrite <- E. apply eqb_list_refl. apply H.
Qed. 
(** [] *)
...
Proof.
  ...
  - induction l as [|x l' IHl'].
    ...
    + simpl. intros H. apply andb_true_iff in H.
      destruct H as [Hx Hl'].
      split. apply Hx. apply IHl' in Hl'. apply Hl'.
  - induction l as [|x l' IHl'].
    + simpl. reflexivity.
    + simpl. intros H. 
      destruct H as [Hx Hl'].
      rewrite Hx. simpl. apply IHl' in Hl'. apply Hl'.
Qed.
...
Lemma rev_append_eq : forall X (l1 l2 l3 : list X), 
  rev_append l1 (l2 ++ l3) = rev_append l1 l2 ++ l3.
Proof.
  intros X l1.
  induction l1 as [|x l1' IHl1'].
  - simpl. reflexivity.
  - simpl. intros l2 l3. apply (IHl1' (x :: l2) l3).
Qed.
...
Proof. 
  ...
  assert (H2 : P -> False).
  { intros Q. apply (fun x => H (or_introl x)) in Q. apply Q. }
...
Proof.
  ...
  intros x.
  destruct (excluded_middle (P x)) as [H1 | H2].
  - apply H1.
  - exfalso. apply H. exists x. apply H2.
Qed.
...
Lemma implies_to_or_rev : forall P Q:Prop,
  (~P \/ Q) -> (P -> Q).
Proof.
  intros P Q H.
  destruct H as [H | H].
...
destruct (excluded_middle P) as [H1 | H1].
- destruct (excluded_middle Q) as [H2 | H2].
  + right. apply H2.
  + apply H in H1. right. apply H1.
- left. apply H1.
Qed.
...
Proof.
  ...
  destruct H as [H | H].
  - assert (goal : (P -> Q) <-> (~P \/ Q)).
    { split. apply implies_to_or. apply implies_to_or_rev. }
    rewrite goal in H.
    apply de_morgan_not_or in H. destruct H as [H H'].
    apply implies_to_or_to_excluded_middle in implies_to_or as excluded_middle.
    apply excluded_middle_to_double_negation_elimination in 
      excluded_middle as double_negation_elimination.
    apply double_negation_elimination in H.
    apply H.
  - apply H.
Qed.

(* 1 -> 5 *)
Theorem peirce_to_consequentia_mirabilis :
  peirce -> consequentia_mirabilis.
Proof.
  intros peirce P.
  unfold not. apply peirce.
Qed.
...
Proof.
  intros excluded_middle P Q.
  intros H.
  destruct (excluded_middle P) as [H1 | H1].
  - left. apply H1.
  - destruct (excluded_middle Q) as [H2 | H2].
    + right. apply H2.
    + exfalso. apply H. split. apply H1. apply H2.
Qed.

(* 5 -> 2 *)
Theorem consequentia_mirabilis_to_double_negation_elimination :
  consequentia_mirabilis -> double_negation_elimination.
Proof.
  intros consequentia_mirabilis P.
  unfold not in *.
  intros H.
...
Lemma idempotent_and : forall P : Prop, P /\ P <-> P.
Proof.
  intros P. split. 
...
(* 3 -> 2 *)
Theorem de_morgan_not_and_not_to_double_negation_elimination :
  de_morgan_not_and_not -> double_negation_elimination.
Proof.
  intros de_morgan_not_and_not P. 
  assert (goal : ~(~P /\ ~P) -> (P \/ P)).
  { apply de_morgan_not_and_not. }
  rewrite idempotent_or in goal.
  rewrite idempotent_and in goal.
  apply goal.
Qed.


Theorem double_negation_elimination_to_implies_to_or :
  double_negation_elimination -> implies_to_or.
Proof.
  intros double_negation_elimination.
  intros P Q H.
  assert (goal : (Q \/ ~Q)).
  { apply double_negation_elimination. apply excluded_middle_irrefutable. }
  destruct goal as [HQ | NHQ].
  - right. apply HQ.
  - left. intros HP. apply H in HP. 
